...
// TestNbioWebsocketRoundTrip validates that the vendored nbio websocket server
// still upgrades HTTP connections, delivers messages, and fires the
...
func TestNbioWebsocketRoundTrip(t *testing.T) {
	...
	// backed by llib's crypto/tls which nbio requires).
